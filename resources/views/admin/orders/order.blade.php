        ...
        <div class="col-md-12">
            <div class="card"  >
                <div class="card-header d-flex justify-content-between @if ($order->completed) text-white bg-secondary @endif">
                    <p class="h3">@if ($order->completed) <span class="p-1 bg-success rounded">Завершён</span> @endif Детали заказа №<strong>{{ $order->number }}</strong> от {{ $order->create_d_m_y_t }} (покупатель: 
                        @if (isset($order->consumers->id))
                            <a href="{{ route('admin.consumer', $order->consumers->id) }}">{{ $order->consumers->full_name ?? '' }}</a>
                        @else
